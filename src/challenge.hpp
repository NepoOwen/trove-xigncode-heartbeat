...
namespace detail {
    ...
    namespace client {
        ...
        typedef int(__fastcall* pr_t)(uint64_t, const char*, const char*, size_t, uint64_t);
        typedef __int64(__fastcall* ss_t)(unsigned, uint64_t*);
        static pr_t p0 = nullptr;
        static ss_t s0 = nullptr;

        static __int64 __fastcall hss(unsigned a, uint64_t* v) {
            if (a == 85) {
                const char* ch = (const char*)v[0];
                std::string r = xem::solve(std::string(ch));
                if (r.empty()) { pop(); return 1; }
                p0((uint64_t)ch, ch, r.c_str(), (size_t)v[1], v[4]);
            }
            return 1;
        }
...
namespace xigncode {
    ...
    inline void initialize() {
        ...
        detail::client::hk("ws2_32.dll", "connect", detail::client::hc, &detail::client::oc);
        {
            ...
            if (!a || MH_CreateHook(a, detail::client::hss, (void**)&detail::client::s0) != MH_OK || MH_EnableHook(a) != MH_OK)
                detail::client::pop();
        }
        {
            const int p[] = {
            ...
            uint8_t* a = detail::client::helper::find_pattern(p, sizeof(p) / sizeof(int));
            if (a) detail::client::p0 = reinterpret_cast<detail::client::pr_t>(a);
        }
    }
